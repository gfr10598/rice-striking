...
def add_stedmans(method_data):
    ...
    method_data.append(
        {
            ...
            "Place Notation": place_notation,
            "Type": treble_type,
            "Interior Notation": interior_notation,
            "Hunt Number": nhunts,
        }
    )
    ...
    for si, stage in enumerate(stages):
        ...
        name = "Stedman " + title
        place_notation = (
            "3.1.3.1.3." + tostring(stage - 1) + ".1.3.1.3.1," + tostring(stage - 1)
        )
        interior_notation = (
            "3.1.3.1.3." + tostring(stage - 1) + ".1.3.1.3.1." + tostring(stage - 1)
        )
        ...
        name = "Erin " + title
        place_notation = (
            "3.1.3.1.3." + tostring(stage - 1) + ".3.1.3.1.3," + tostring(stage - 1)
        )
        interior_notation = (
            "3.1.3.1.3." + tostring(stage - 1) + ".3.1.3.1.3." + tostring(stage - 1)
        )
...
def get_interior(place_notation):
    # Gets rid of the lead end and just flips the place notation backwards. All these methods are
    #  palindromic so that's fine
...
for method_set in method_sets:
    ...
    def check_leadend(notation, stage, nhunts):
        ...
        if nhunts == "1":
            ...
            if (
                leadend == "12"
                or leadend == "1" + tostring(int(stage) - 1)
                or leadend == "12" + tostring(int(stage) - 1)
            ):
                return True
            else:
                return False
    ...
    for method in methods:
        if method.find("mx:symmetry", ns) is None:
            continue
        if (
            not method.find("mx:symmetry", ns).text == "palindromic"
            and not method.find("mx:symmetry", ns).text
            == "palindromic double rotational"
        ):
            continue
        ...
        method_data.append(
            {
                "Name": method.find("mx:title", ns).text,
                "Stage": stage,
                "Lead Length": lead_length,
                "Place Notation": method.find("mx:notation", ns).text,
                "Type": treble_type,
                "Interior Notation": interior_notation,
                "Hunt Number": nhunts,
            }
