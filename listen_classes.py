...
class audio_data:
    ...
    def __init__(self, raw_file, doprints=True):
        ...
        if ext != ".wav":
            ...
            os.system(
                "ffmpeg -y -loglevel quiet -i ./tmp/%s ./tmp/%s.wav"
                % (raw_file.name, raw_file.name[:-4])
            )
        ...
        if upload_success:
            ...
            with warnings.catch_warnings():
                ...
                self.fs, self.data = wavfile.read(
                    new_fname
                )  # Hopefully this doesn't use too much...
...
class parameters(object):
    ...
    def __init__(
        ...
        self.nrounds_min = (
            8  # How many rounds do you need (8 = 4 whole pulls, seems reasonable...)
        )
        ...
        self.rounds_tcut = (
            0.5  # How many times the average cadence to cut off find in rounds
        )
        self.rounds_leeway = (
            1.5  # How far to allow a strike before it is more improbable
        )
        ...
        if True:  # not st.session_state.trim_flag:
            if overall_tmax > 0.0:
                st.session_state.trimmed_signal = st.session_state.audio_signal[
                    round(overall_tmin * st.session_state.fs) : round(
                        overall_tmax * st.session_state.fs
                    )
                ]
            else:
                st.session_state.trimmed_signal = st.session_state.audio_signal[
                    round(overall_tmin * st.session_state.fs) :
                ]
        ...
        self.fcut_int = 2 * round(
            self.fcut_length * st.session_state.fs / 2
        )  # Length of this cut (must be even for symmetry purposes)
        self.tmax = (
        ...
        self.frequency_skew = (
            2.0  # How much to favour the high frequencies for timing reasons
        )
...
class data:
    ...
    def __init__(self, Paras, tmin=-1, tmax=-1):
        ...
        st.session_state.local_signal = st.session_state.trimmed_signal[
            cut_min_int:cut_max_int
        ]

        self.nominals = Paras.nominals

        self.initial_profile = np.identity(
            Paras.nbells
        )  # Initial frequencies for the bells -- these are just the nominals
        ...
        self.frequency_profile = np.identity(
            Paras.nbells
        )  # Each bell corresponds to its nominal frequency alone -- this will later be updated.
