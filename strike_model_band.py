...
from scipy.optimize import minimize_scalar, curve_fit  # type: ignore
...
def find_all_gaps_band(alltimes, nbells, nrows):
    all_gaps = np.zeros(
        nrows
    )  # Handstroke gaps BEFORE each row. Backstrokes will just be zero
    ...
    for row in range(1, nrows - 1, 1):  # Starting the cut on each backstroke
        ...
        cut = np.array(
            alltimes[start:end]
        )  # Contains 24 blows with a backstroke change and handstroke change
...
def find_ideal_times_band(alltimes, nbells, ncount=24, ngaps=6, key=-1):
    ...
    all_predicted_gaps = find_predicted_gaps_band(
        all_ideal_gaps, nbells, nrows, ngaps=ngaps
    )

    # print('Handstroke gaps determined...')
    # Actually finds the ideal strike time of each bell, based on the predicted handstroke gaps and
    #  the preceding strikes (up to a point to be determined.)
    ...
    def adjust_times(data, row_number, n_adjust, position, cut_start_distance):
        # Remove the effect of the handstroke gaps -- keep the individual times the same but
        #  rewrite the preceding few as necessary, essentially assuming everything before comes
        #  earlier
    ...
    for strike in range(len(all_ideals)):
        ...
        data = adjust_times(
            data, row_number, n_adjust, row_position, strike - back
        )  # Adjust to take into account handstroke gaps
