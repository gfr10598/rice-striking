...
def find_ringing_times(Paras, Data):
    ...
    loudsmooth[0] = 0.0
    loudsmooth[-1] = 0.0  # For checking peaks

    threshold = np.max(loudsmooth) * 0.8
    # Use this to determine the start time of the ringing -- time afte
    peaks, _ = find_peaks(
        loudsmooth, width=round(10.0 / Paras.dt)
    )  # Prolonged peak in noise - probably ringing
...
def find_first_strikes(Paras, Data):
    ...
    rough_cadence = find_rough_cadence(
        Paras, Data
    )  # This gives an impression of how long it is between successive changes
    ...
    npeaks_ish = int(
        (len(Data.strike_probabilities[0]) - Paras.ringing_start * Paras.dt)
        / rough_cadence
    )
    ...
    for bell in range(nbells):  # Establish peaks and reliability
        ...
        peaks = np.array(
            [val for _, val in sorted(zip(prominences, peaks), reverse=True)]
        ).astype("int")
        prominences = np.array(sorted(prominences, reverse=True))
        if len(peaks) > 2 * npeaks_ish:
            # This will work for comparisons
            confidence_ratio = np.mean(prominences[:npeaks_ish]) / np.mean(
                prominences[npeaks_ish : 2 * npeaks_ish]
            )
        else:
            confidence_ratio = 0
        ...
        threshold = (
            0.9
            * sorted(prominences, reverse=True)[
                min(int(0.75 * npeaks_ish), len(peaks) - 1)
            ]
        )
    ...
    for bell in range(nbells):
        ...
        if shift > 0:
            probability_shift[shift:] = (
                probability_shift[shift:]
                + Data.strike_probabilities[bell][:-shift]
                * ((bell + 1) / nbells) ** alpha
            )
            ...
            probability_shift[:] = (
                probability_shift[:]
                + Data.strike_probabilities[bell] * ((bell + 1) / nbells) ** alpha
            )
    ...
    shiftpeaks = np.array(
        [val for _, val in sorted(zip(shiftproms, shiftpeaks), reverse=True)]
    ).astype("int")
    ...
    for pi in range(0, len(significant_peaks)):
        if pi == 0:
            ...
            nearest_distance = min(
                abs(significant_peaks[pi - 1] - significant_peaks[pi]),
                abs(significant_peaks[pi + 1] - significant_peaks[pi]),
            )
    ...
    if len(lonesome_peaks) < 4:
        st.error(
            "Not found reliable enough rounds. Apologies. Try trimming audio from the start?"
        )
        if st.session_state.testing_mode:
            test_error(
                "Not found reliable enough rounds. Apologies. Try trimming audio from the start?"
    ...
    for li in range(1, len(lonesome_peaks)):
        if lonesome_peaks[li] < 15.0 / Paras.dt:  # Adjust for slow pull-off
            gap = int(
                round(
                    (lonesome_peaks[li] - lonesome_peaks[li - 1]) / rough_cadence
                    - 0.25,
                    0,
                )
            )
            ...
            gap = int(
                round((lonesome_peaks[li] - lonesome_peaks[li - 1]) / rough_cadence, 0)
            )

        for k in range(max(0, gap - 1)):
            first_guesses.append(
                lonesome_peaks[li - 1]
                + (k + 1) * (lonesome_peaks[li] - lonesome_peaks[li - 1]) / gap
            )
    ...
    rough_cadence = np.mean(
        np.array(sorted(first_guesses))[1:] - np.array(sorted(first_guesses)[:-1])
    )
    ...
    diff1s = (
        np.array(first_guesses)[2:][1:nrows_check:2]
        - np.array(first_guesses)[2:][0 : nrows_check - 1 : 2]
    )
    diff2s = (
        np.array(first_guesses)[2:][2 : nrows_check + 1 : 2]
        - np.array(first_guesses)[2:][1:nrows_check:2]
    )
    ...
    for li in range(1, len(lonesome_peaks)):
        ...
        if gap > 1:
            ...
            start = lonesome_peaks[li - 1]
            end = lonesome_peaks[li]
            position = start
            if handstroke:
                total_interbell_gaps = (nbells * 2 + 1) * (gap // 2) + (gap % 2) * (
                    nbells + 1
                )
            else:
                total_interbell_gaps = (nbells * 2 + 1) * (gap // 2) + (gap % 2) * (
    ...
    if (
        ...
        and stroke_difference * Paras.dt < 10.0
    ):
        handstroke_first = True
    ...
    for strike in second_guesses:
        ...
        if len(options) == 0:
            ...
            prom_options = peak_prominences(
                gaussian_filter1d(probability_shift, 3), options
            )[0]
    ...
    belltimes = np.linspace(
        final_guesses[0] - cadence_guess * (nbells - 1), final_guesses[0], Paras.nbells
    )
    ...
    for ri in range(0, len(final_guesses) - 1):
        ...
        if handstroke:
            belltimes = np.linspace(
                final_guesses[ri], final_guesses[ri + 1], Paras.nbells + 1
            )
        else:
            belltimes = np.linspace(
                final_guesses[ri], final_guesses[ri + 1], Paras.nbells + 2
            )
    ...
    spacings = np.mean(cadences) * np.ones(
        Paras.nbells
    )  # Distances from adjacent bells
    ...
    for bell in range(Paras.nbells):
        ...
        probs_adjust = (
            probs_raw[bell, :] ** (Paras.probs_adjust_factor + 1)
            / (np.sum(probs_raw[:, :], axis=0) + 1e-6) ** Paras.probs_adjust_factor
        )
        ...
        for ri in range(Paras.nrounds_max):
            ...
            poss = peaks[
                (peaks > aim - 1.0 * cadence) * (peaks < aim + 1.0 * cadence)
            ]  # These should be accurate strikes
            yvalues = sigs[
                (peaks > aim - 1.0 * cadence) * (peaks < aim + 1.0 * cadence)
            ]
    ...
    for ri in range(Paras.nrounds_max):
        for bell in range(0, Paras.nbells):
            if bell == 0:
                ...
                spacings[bell] = min(
                    strikes[bell + 1, ri] - strikes[bell, ri],
                    strikes[bell, ri] - strikes[bell - 1, ri],
                )
...
def find_strike_times(Paras, Data, final=False):
    ...
    strike_probs_adjust = (
        strike_probs[:, :] ** (Paras.probs_adjust_factor + 1)
        / (np.sum(strike_probs[:, :], axis=0) + 1e-6) ** Paras.probs_adjust_factor
    )

    strike_probs_adjust = gaussian_filter1d(
        strike_probs_adjust, Paras.rounds_probs_smooth, axis=1
    )
    ...
    for bell in range(Paras.nbells):
        ...
        probs_smooth = 1.0 * gaussian_filter1d(
            probs, round(Paras.smooth_time / Paras.dt)
        )
        ...
        bigpeaks = peaks[
            prominences > 0.5 * probs_smooth[peaks]
        ]  # For getting first strikes, need to mbe more significant
    ...
    if len(Paras.allstrikes) == 0:
        ...
        change_start = np.mean(Data.last_change) - Data.cadence_ref * (
            (Paras.nbells - 1) / 2
        )
        change_end = np.mean(Data.last_change) + Data.cadence_ref * (
            (Paras.nbells - 1) / 2
        )
        ...
        if not handstroke:
            ...
            taims = np.array(Data.last_change) + round(
                (Paras.nbells + 1) * Data.cadence_ref
            )
    ...
    while go:
        ...
        sigs_range = []
        if (
            len(Paras.allstrikes) == 0 and len(allstrikes) < 4
        ):  # Establish first strikes overall.
            ...
            for bell in range(Paras.nbells):  # This is a bit shit -- improve it?
                ...
                poss = allpeaks[bell][
                    (allpeaks[bell] > start_bell) * (allpeaks[bell] < end_bell)
                ]
                sigposs = allsigs[bell][
                    (allpeaks[bell] > start_bell) * (allpeaks[bell] < end_bell)
                ]

                poss = np.array(
                    [val for _, val in sorted(zip(sigposs, poss), reverse=True)]
                )
            ...
            for bell in range(Paras.nbells):
                ...
                start_bell = taims[bell] - round(
                    3.5 * Paras.cadence
                )  # Aim within the change
                end_bell = taims[bell] + round(3.5 * Paras.cadence)
                # Check physically possible...
                if len(allstrikes) == 0:
                    start_bell = max(
                        start_bell, Data.last_change[bell] + round(3.0 * Paras.cadence)
                    )
                else:
                    start_bell = max(
                        start_bell, allstrikes[-1][bell] + round(3.0 * Paras.cadence)
                    )

                sigs_range = sigs_range[
                    (peaks_range > start_bell) * (peaks_range < end_bell)
                ]
                ...
                if len(peaks_range) == 1:  # Only one time that it could reasonably be
                    ...
                    tvalue = (
                        1.0
                        / (abs(peaks_range[0] - taims[bell]) / tcut + 1)
                        ** Paras.strike_alpha
                    )
                    if final:
                        confs[bell] = 1.0
                    ...
                    for k in range(len(peaks_range)):  # Many options...
                        if abs(peaks_range[k] - taims[bell]) < round(
                            Paras.rounds_leeway * Paras.cadence
                        ):
                            tvalue = 1.0
                        else:
                            tvalue = 1.0 / (
                                abs(
                                    abs(peaks_range[k] - taims[bell])
                                    - round(Paras.rounds_leeway * Paras.cadence)
                                )
                                / tcut
                                + 1
                            ) ** (Paras.strike_alpha)
                    ...
                    for k in range(len(peaks_range)):  # Many options...
                        tvalue = (
                            1.0
                            / (abs(peaks_range[k] - np.mean(taims)) / tcut + 1)
                            ** Paras.strike_alpha
                        )
            ...
            if np.median(certs) < 0.01:
                ...
                Data.freq_data = np.array([Paras.dt, Paras.fcut_length, 0.0, 0.0])
                Data.freq_data = np.concatenate(
                    (Data.freq_data, np.zeros(Paras.nbells))
                )
        ...
        if handstroke:
            ...
            taims = np.array(allstrikes[-1]) + round(
                (Paras.nbells + 1) * Data.cadence_ref
            )
            next_start = change_start + round((Paras.nbells + 1) * Data.cadence_ref)
            next_end = change_end + round((Paras.nbells + 1) * Data.cadence_ref)
    ...
    if len(allconfs) > 1:
        ...
        bellconfs_individual = np.mean(np.array(allconfs)[1:, :], axis=0)
        Data.freq_data = np.array(
            [Paras.dt, Paras.fcut_length, np.mean(allconfs[1:]), np.min(allconfs[1:])]
        )
        ...
        for si in range(len(row)):
            if si == 0:
                if ri == 0:
                    ...
                    spacings[ri, order[si], 0] = row[order[si]] - np.max(
                        allstrikes[ri - 1]
                    )
            else:
            ...
            if si == len(row) - 1:
                if ri == len(allstrikes) - 1:
                    ...
                    spacings[ri, order[si], 1] = (
                        np.min(allstrikes[ri + 1]) - row[order[si]]
                    )
            else:
    ...
    if nstrikes_done < len(Paras.first_strikes[0]):
        ...
        Paras.first_strikes[:, :] = np.array(allstrikes).T[
            :, : len(Paras.first_strikes[0])
        ]
...
def check_for_misses(allstrikes, allcerts, last_switch):
    ...
    for ri, row in enumerate(allrows):
        ...
        prev_order = order
    if (
        ndiffs / len(allrows) < 0.2
    ):  # This is probably just rounds and calls -- leave it be
        return allstrikes, allcerts, last_switch

    # Try running through to get an overall maxcounts
    maxcounts = np.zeros(
        (len(allrows), nbells), dtype=int
    )  # Counts of bells in each position (other than rounds)
    prev_order = yvalues
    orders = []
    lengths = []
    for ri, row in enumerate(allrows):
        ...
        if (
            not (order == yvalues).all()
        ) and ri > 2:  # Obviously rounds is probably fine
    ...
    def lookfor_rounds(min_index, static_counts, allrows):
        ...
        if np.max(static_counts) > 2:
            ...
            for fi_rounds, roundsbutnot_count in enumerate(static_counts):
                if (
                    roundsbutnot_count > 2 and fi_rounds >= min_index
                ):  # In the same position for three blows. Take this one as the first occasion.
                    isnotrounds = True
        ...
        if isnotrounds:
            reference_rows = allrows[
                start_index:end_index
            ]  # Take one fewer than the end in case this does go as far as the end
            ...
            if np.sum(bells_after) < nbells / 2:
                ...
                shift_rows[:, bells_after] = allrows[
                    start_index - 1 : end_index - 1, bells_after
                ]
            else:
            ...
            for rir, row in enumerate(shift_rows):
                new_orders[rir] = np.array(
                    [val for _, val in sorted(zip(row, yvalues), reverse=False)]
                )
            ...
            if roundscount / (len(shift_rows) - 1) > 1.0:
                ...
                return (
                    ...
                    start_index,
                    shift_rows[0],
                )  # Flag for genuine shift, index if so and the rows which have shifted
            else:
    ...
    def lookfor_front(min_index, maxcounts, allrows):
        ...
        if np.max(maxcounts[:, 0]) > 2:
            for fi_front, front_count in enumerate(maxcounts[:, 0]):
                if (
                    front_count > 2 and fi_front >= min_index
                ):  # In the same position for three blows. Take this one as the first occasion.
                    isfront = True
        ...
        if isfront:
            ...
            shift_rows = reference_rows.copy()
            shift_rows[:, shift_bell] = allrows[
                start_index + 1 : end_index + 1, shift_bell
            ]
    ...
    def lookfor_back(min_index, maxcounts, allrows):
        ...
        if np.max(maxcounts[:, -1]) > 2:
            for fi_back, back_count in enumerate(maxcounts[:, -1]):
                if (
                    back_count > 2 and fi_back >= min_index
                ):  # In the same position for three blows. Take this one as the first occasion.
                    isback = True
    ...
    while gorounds:
        ...
        if (
            ...
            switch_index > 0
        ):  # There was a potential switch but no good, so keep looking
    ...
    if rounds_switch_flag and rounds_switch_test <= min(
        back_switch_index, front_switch_index
    ):
    ...
    elif back_switch_flag and back_switch_index <= min(
        front_switch_index, rounds_switch_test
    ):
        # print('Switch back')
    ...
    elif front_switch_flag and front_switch_index <= min(
        back_switch_index, rounds_switch_test
    ):
        # print('Switch front')
...
def do_frequency_analysis(Paras, Data):
    ...
    for bell in range(Paras.nbells):
        max_cert_threshold = sorted(Data.strike_certs[bell, :])[
            -4
        ]  # Get rid of massive outliers
        Data.strike_certs[bell, :] = np.minimum(
            Data.strike_certs[bell, :], max_cert_threshold
        )
    ...
    for si in range(nstrikes):
        ...
        tmin = round(np.min(strikes) - Paras.cadence * (Paras.nbells - 2))
        tmax = round(np.max(strikes) + Paras.cadence * (Paras.nbells - 2))
        ...
        for fi, freq_test in enumerate(freq_tests):
            ...
            sigs = prominences / np.max(
                prominences
            )  # Significance of the peaks relative to the background flow
            ...
            for bell in range(Paras.nbells):
                ...
                for pi, peak_test in enumerate(peaks):
                    tvalue = (
                        1.0
                        / (abs(peak_test - strikes[bell]) / tcut + 1)
                        ** Paras.strike_alpha
                    )
                    best_value = max(
                        best_value,
                        sigs[pi] ** Paras.strike_gamma_init * tvalue * pvalue,
                    )
    ...
    best_probs = (
        []
    )  # Pick out the probabilities for each bell for each of these frequencies
    ...
    for bell in range(Paras.nbells):
        ...
        probs_clean = gaussian_filter1d(
            probs_clean, Paras.freq_filter
        )  # Stops peaks wiggling around. Can cause oscillation in ability.
        ...
        peaks = peaks[
            peaks > 50
        ]  # Data.nominals[bell]*1.1]  #Use nominal frequencies here?
    ...
    for bell in range(Paras.nbells):
        ...
        freqs_arrange = np.array(
            [
                val
                for _, val in sorted(
                    zip(best_probs[:, bell], final_freqs), reverse=True
                )
            ]
        ).astype("int")
        ...
        for fi in freqs_arrange[: nfinals * 2]:
            ...
            others = final_freqs[
                (abs(final_freqs - fi) < 20) * (abs(final_freqs - fi) > 0)
            ]
...
def find_strike_probabilities(Paras, Data, init=False, final=False):
    ...
    for fi, freq_test in enumerate(Data.test_frequencies):
        ...
        if init:
            diffpeaks = diffpeaks[
                prominences > diffsum_smooth[diffpeaks]
    ...
    if init:
        ...
        for bell in range(Paras.nbells):
                    ...
                    if np.max(sigs) > 0.1:  # Maybe this is harsh but it should work...
                        ...
                        for k in range(len(sigs)):
                            final_probs = final_probs + [
                                Data.frequency_profile[fi, bell]
                            ]
            ...
            tvalues = (
                1.0
                / (np.abs(final_poss[:, np.newaxis] - t_ints[np.newaxis, :]) / tcut + 1)
                ** Paras.strike_alpha
            )
            ...
            allvalues = (
                tvalues
                * final_sigs[:, np.newaxis] ** Paras.prob_beta
                * final_probs[:, np.newaxis] ** Paras.strike_gamma
                * fshift[:, np.newaxis]
            )
            ...
            overall_probs_smooth = gaussian_filter1d(
                overall_probs, round(Paras.smooth_time / Paras.dt), axis=0
            )
