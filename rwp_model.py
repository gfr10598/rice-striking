...
# This one is for the RWP (Rod Pipe) model, using the principle of the code provided by
#  Ian McCallion written in OCtave (but completely rewritten)
...
def find_centres(alltimes_trim, nbells):
    ...
    for wholepull in range(1, nwholepulls - 1):
        ideal_centres[wholepull] = 0.5 * (
            centres[wholepull + 1] + centres[wholepull - 1]
        )
        ideal_durations[wholepull] = 0.5 * (
            centres[wholepull + 1] - centres[wholepull - 1]
        )
    ideal_durations[0] = ideal_durations[1]
    ideal_durations[-1] = ideal_durations[-2]
...
def find_all_times(nbells, ideal_centres, ideal_interbell_gaps):
    ...
    for wholepull in range(nwholepulls):
        row_times = ideal_centres[wholepull] + ideal_interbell_gaps[wholepull] * (
            np.arange(nbells * 2) - nbells + 0.5
        )
...
def find_ideal_times_rwp(alltimes, nbells, key=-1):
    ...
    # Finds the ideal times based on the Rod Pipe model. Requires a whole number of whole pulls,
    #  which won't necessarily be the case for me
    ...
    avg_interbell_gap = np.mean(wholepull_limits[:, 1] - wholepull_limits[:, 0]) / (
        nbells * 2 - 1
    )
    avg_handstroke_gap = np.mean(wholepull_limits[1:, 0] - wholepull_limits[:-1, 1])
    gap_ratio = avg_handstroke_gap / avg_interbell_gap

    ideal_centres, ideal_durations = find_centres(
        alltimes_trim, nbells
    )  # The ideal centres and durations of each whole pull
