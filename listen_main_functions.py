...
from listen_other_functions import (
    find_ringing_times,
    find_strike_probabilities,
    do_frequency_analysis,
    find_strike_times,
    find_colour,
    check_initial_rounds,
    find_first_strikes,
    test_error,
    check_for_misses,
)
...
def establish_initial_rhythm(Paras, final=False):
    ...
    if not final:
        Data = data(
            Paras, tmin=0.0, tmax=Paras.reinforce_tmax
        )  # This class contains all the important stuff, with outputs and things
    else:
        Data = data(
            Paras, tmin=0.0, tmax=60.0
        )  # This class contains all the important stuff, with outputs and things
    ...
    if not final:
        st.current_log.write(
            "Ringing detected from approx. time %d seconds"
            % (Paras.ringing_start * Paras.dt)
        )

    if not final:
        Data = data(
            Paras, tmin=Paras.ringing_start * Paras.dt, tmax=Paras.reinforce_tmax
        )  # This class contains all the important stuff, with outputs and things
    else:
        Data = data(
            Paras,
            tmin=Paras.ringing_start * Paras.dt,
            tmax=60.0 + Paras.ringing_start * Paras.dt,
        )  # This class contains all the important stuff, with outputs and things

    # Find strike probabilities from the nominals
    Data.strike_probabilities = find_strike_probabilities(
        Paras, Data, init=True, final=final
    )
    # Find the first strikes based on these probabilities. Hopefully some kind of nice pattern
    #  to the treble at least...
    Paras.local_tmin = Paras.overall_tmin
    Paras.local_tint = int(Paras.overall_tmin / Paras.dt)
    Paras.stop_flag = False

    # Paras.ringing_start, Paras.ringing_end = find_ringing_times(Paras, Data)  #Why does this
    #  happen twice? Forgot...
...
def do_reinforcement(Paras, Data):
    ...
    # Find the first strikes based on these probabilities. Hopefully some kind of nice pattern to
    #  the treble at least...
    ...
    for reinforce_count in range(Paras.n_reinforces):
        ...
        # Find the probabilities that each frequency is useful. Also plots frequency profile of
        #  each bell, hopefully.
        # st.write('Doing frequency analysis,  iteration number', count + 1, 'of',
        #           Paras.n_reinforces)
        st.main_log.write(
            "**Learning bell frequencies, iteration %d**" % (reinforce_count + 1)
        )

        Data.test_frequencies, Data.frequency_profile = do_frequency_analysis(
            Paras, Data
        )
        ...
        Data.strike_probabilities = find_strike_probabilities(
            Paras, Data, init=False, final=False
        )
        Data.end_flag = False

        strikes, strike_certs = find_strike_times(
            Paras, Data, final=False
        )  # Finds strike times in integer space
        ...
        if not all_is_well:
            st.error(
                "Failed to detect rounds. Either there aren't any"
                " or the recording isn't good enough..."
            )
        ...
        # Estalish if the strokes might be the wrong way around... Look at diffs as in initial
        #  rounds.
        ...
        while not done:
            for bell in range(Paras.nbells):
                ...
                if len(allcerts) > Paras.nreinforce_rows:
                    threshold = max(
                        threshold, sorted(allcerts, reverse=True)[Paras.nreinforce_rows]
                    )
                for row in range(len(strikes[0])):
                    if (
                        strike_certs[bell, row] > threshold
                        and count < Paras.nreinforce_rows
                    ):
        ...
        st.current_log.write(
            "Using " + str(len(Data.strikes[0])) + " rows for next reinforcement"
        )
        ...
        if len(Data.strikes) > 0 and len(Data.strike_certs) > 0:
            # Check if it's worth overwriting the old one? Do this at EVERY STEP, and save out
            #  to THIS filename.
            update = False
            ...
            st.error(
                "Frequency analysis failed for some reason. If the percentage is"
                " reasonably high this is probably due to bad audio or bad ringing."
                " If it is low then check the correct tower/bells are selected."
            )
            if st.session_state.testing_mode:
                test_error(
                    "Frequency analysis failed for some reason. If the percentage is"
...
def find_final_strikes(Paras, nested=False):
    ...
    st.analysis_sublog.progress(
        0, text="Initial rhythm established, finding all strikes"
    )
    ...
    while not Paras.stop_flag and not Paras.ringing_finished:
        ...
        if counter == 0:
            ...
            Paras.first_strikes = (
                Paras.first_strikes + Paras.ringing_start - round(tmin / Paras.dt)
            )

        Data.strike_probabilities = find_strike_probabilities(
            Paras, Data, init=False, final=True
        )
        ...
        Data.strikes, Data.strike_certs = find_strike_times(
            Paras, Data, final=True
        )  # Finds strike times in integer space
        ...
        if len(Data.strikes) == 0:
            ...
            np.median(Data.strike_certs[:, -1]) < 0.25
        ):  # Can see if this can be fixed... who knows?
            Paras.stop_flag = True
            Paras.ringing_finished = True
        ...
        if len(np.shape(Data.strikes)) > 1:
            ...
            for row in range(0, len(Data.strikes[0])):
                allstrikes.append(
                    (Data.strikes[:, row] + round(tmin / Paras.dt)).tolist()
                )
                allcerts.append(Data.strike_certs[:, row].tolist())
                allcadences.append(
                    (np.max(allstrikes[-1]) - np.min(allstrikes[-1]))
                    / (Paras.nbells - 1)
                )
        ...
        if len(length_log) > 0:
            if len(allstrikes) != length_log[-1]:
                allstrikes, allcerts, switch_id = check_for_misses(
                    allstrikes, allcerts, last_switch
        ...
        progress_fraction = (
            np.max(allstrikes[-1]) * Paras.dt - Paras.ringing_start * Paras.dt
        ) / (len(st.session_state.trimmed_signal) / st.session_state.fs)
...
def filter_final_strikes(Paras):
    ...
    for ri in range(len(change_ratios[0])):
        change_ratios[:, ri] = (
            st.session_state.allstrikes[:, ri]
            - np.min(st.session_state.allstrikes[:, ri])
        ) / (
            np.max(st.session_state.allstrikes[:, ri])
            - np.min(st.session_state.allstrikes[:, ri])
        )
    ...
    for bell in range(len(change_ratios)):
        for ri in range(1, len(change_ratios[0]) - 1):
            if (
                st.session_state.allcerts[bell, ri] < 0.1
            ):  # This is one to be interpolated. Hopefully those either side are fine...
                predicted_ratio = 0.5 * (
                    change_ratios[bell, ri - 1] + change_ratios[bell, ri + 1]
                )
                predicted_location = np.min(
                    st.session_state.allstrikes[:, ri]
                ) + predicted_ratio * (
                    np.max(st.session_state.allstrikes[:, ri])
                    - np.min(st.session_state.allstrikes[:, ri])
    ...
    penultimate_order = np.array(
        [
            val
            for _, val in sorted(
                zip(st.session_state.allstrikes[:, -2], yvalues), reverse=False
            )
        ]
    )
    ultimate_order = np.array(
        [
            val
            for _, val in sorted(
                zip(st.session_state.allstrikes[:, -1], yvalues), reverse=False
            )
        ]
    )
    ...
    # Check timing
    diffs = (
        np.sort(st.session_state.allstrikes[:, -1])[1:]
        - np.sort(st.session_state.allstrikes[:, -1])[:-1]
    )
    if np.min(diffs) < 1:
        st.session_state.allstrikes = st.session_state.allstrikes[:, :-1]
...
def save_strikes(Paras):
        ...
        for row in range(len(st.session_state.allstrikes[0])):
            ...
            certs = np.array(
                [
                    ...
                    for _, val in sorted(
                        zip(
                            st.session_state.allstrikes[:, row],
                            st.session_state.allcerts[:, row],
                        ),
                        reverse=False,
                    )
                ]
    ...
    striking_df = pd.DataFrame(
        {"Bell No": allbells, "Actual Time": allstrikes, "Confidence": allcerts_save}
    )
