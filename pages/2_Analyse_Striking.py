...
from data_visualisations import (
    calculate_stats,
    obtain_striking_markdown,
    plot_errors_time,
    plot_bar_charts,
    plot_histograms,
    plot_boxes,
    ...
    for uploaded_file in st.session_state.uploaded_files:
        ...
        if uploaded_file.name[-4:] != ".csv":
            ...
            try:
                ...
                if (
                    "Bell No" not in raw_data.columns
                    or "Actual Time" not in raw_data.columns
                ):
                    isfine = False
...
if "cached_rawdata" not in st.session_state:
    st.session_state.cached_rawdata = [
        [] for val in range(len(st.session_state.cached_data))
    ]  # This will contain a dataframe, if one exists.
if "current_touch" not in st.session_state:
    st.session_state.current_touch = (
        -1
...
def make_longtitle_cache(ti):
    ...
    longtitle = ""
    if (
        st.session_state.cached_nchanges[ti] != ""
        and st.session_state.cached_methods[ti] != ""
    ):
        longtitle = longtitle + "%d %s" % (
            int(st.session_state.cached_nchanges[ti]),
            st.session_state.cached_methods[ti],
        )
    else:
        return None
    if st.session_state.cached_tower[ti] != "":
        longtitle = longtitle + " at %s" % (st.session_state.cached_tower[ti])
    longtitle = longtitle + " (%s)" % get_nice_date(
        st.session_state.cached_datetime[ti]
    )
...
st.session_state.url_collection = determine_collection_from_url(
    st.session_state.existing_names
)
...
if st.session_state.url_collection is not None:
    ...
    if (
        ...
        st.session_state.saved_index_list = np.loadtxt(
            "./saved_touches/%s/index.csv" % st.session_state.current_collection_name,
            delimiter=";",
            dtype=str,
        )
    else:
        st.session_state.saved_index_list = np.array(
            [[" ", " ", " ", " ", " ", " ", " ", " "]], dtype="str"
        )
    if len(np.shape(st.session_state.saved_index_list)) == 1:
        st.session_state.saved_index_list = np.array(
            [st.session_state.saved_index_list]
        )
    ...
    add_collection_to_cache(
        st.session_state.ntouches, st.session_state.saved_index_list
    )
...
if len(st.session_state.touch_titles) > 0:
    st.session_state.selection = st.pills(
        "Choose a touch to analyse:",
        st.session_state.touch_titles,
        default=st.session_state.touch_titles[-1],
    )
...
if len(st.session_state.touch_titles) == 0:
    st.write(
        "No data currently loaded: either upload a .csv file with striking data or generate some using the Analyse Recording page"
    )
    st.stop()
...
st.session_state.current_touch = st.session_state.touch_titles.index(
...
if st.session_state.current_touch < 0:
    ...
    st.write(
        'Analysing ringing from "%s"'
        % st.session_state.touch_titles[st.session_state.current_touch]
    )
...
if st.session_state.collection_status == 0:
    ...
    if (
        st.session_state.cached_touch_id[st.session_state.current_touch]
        not in saved_index_list[:, 0]
    ):
        # Load in csv
        if st.button(
            "Add this touch to collection **%s**"
            ...
            new_list_entry = [
                st.session_state.cached_touch_id[st.session_state.current_touch],
                st.session_state.cached_read_id[st.session_state.current_touch],
                st.session_state.cached_nchanges[st.session_state.current_touch],
                st.session_state.cached_methods[st.session_state.current_touch],
                st.session_state.cached_tower[st.session_state.current_touch],
                st.session_state.cached_datetime[st.session_state.current_touch],
                st.session_state.cached_score[st.session_state.current_touch],
                st.session_state.cached_ms[st.session_state.current_touch],
            ]
            ...
            convert_for_download(
                st.session_state.cached_rawdata[st.session_state.current_touch]
            )
            np.savetxt(
                "./saved_touches/%s/index.csv"
                % st.session_state.current_collection_name,
                np.array(new_index_list, dtype=str),
                fmt="%s",
                delimiter=";",
            )
            st.rerun()
        ...
        st.write(
            "This touch is saved in collection **%s**"
            % st.session_state.current_collection_name
        )

if st.session_state.current_touch >= 0:
    # Write in to a local bit to actually do the analysis
    st.session_state.strikes = st.session_state.cached_strikes[
        st.session_state.current_touch
    ]
    st.session_state.certs = st.session_state.cached_certs[
        st.session_state.current_touch
    ]
    ...
    if len(st.session_state.strikes) == 0:
        # This is from a .csv
        st.session_state.raw_data = st.session_state.cached_rawdata[
            st.session_state.current_touch
        ]
        ...
        st.session_state.existing_models = [
            val
            for val in st.session_state.cols
            if val not in ["Bell No", "Confidence", "Actual Time"]
        ]
        st.session_state.existing_models = [
            val for val in st.session_state.existing_models if val[:7] != "Unnamed"
        ]
        st.session_state.existing_models = [
            val
            for val in st.session_state.existing_models
            if val[:17] != "Corrected Bells"
        ]
        ...
        if "Confidence" not in st.session_state.raw_data.columns.tolist():
            ...
            st.session_state.raw_data["Confidence"] = np.ones(
                len(st.session_state.raw_data["Actual Time"])
            )
        ...
        st.session_state.yvalues = (
            np.arange(
                len(
                    st.session_state.cached_strikes[st.session_state.current_touch][
                        :, 0
                    ]
                )
            )
        ...
        for row in range(
            len(st.session_state.cached_strikes[st.session_state.current_touch][0])
        ):
            st.session_state.order = np.array(
                [
                    val
                    for _, val in sorted(
                        zip(
            ...
            st.session_state.certs = np.array(
                [
                    val
                    for _, val in sorted(
                        zip(
            ...
            st.session_state.allcerts_save = (
                st.session_state.allcerts_save + st.session_state.certs.tolist()
            )
            st.session_state.allbells = (
                st.session_state.allbells + st.session_state.order.tolist()
            )
            st.session_state.orders.append(st.session_state.order)

        st.session_state.allstrikes_flat = (
            1000 * np.array(st.session_state.allstrikes_flat) * 0.01
        )
        ...
        st.session_state.raw_data = pd.DataFrame(
            {
                "Bell No": st.session_state.allbells,
                "Actual Time": st.session_state.allstrikes_flat,
                "Confidence": st.session_state.allcerts_save,
            }
        )
    ...
    (
        ...
        st.session_state.hunt_types,
        st.session_state.calls,
        st.session_state.start_row,
        st.session_state.end_row,
        st.session_state.allrows_correct,
        st.session_state.quality,
    ) = find_method_things(st.session_state.raw_data["Bell No"])
    st.session_state.calling_html = None
    st.session_state.comp_html = None
    st.session_state.call_string = None
    if len(st.session_state.methods) > 0:
        (
            st.session_state.call_string,
            st.session_state.comp_html,
            st.session_state.calling_html,
        ) = print_composition(
            st.session_state.methods,
            st.session_state.hunt_types,
            st.session_state.calls,
    ...
    if len(st.session_state.methods) > 0:
        ...
        st.session_state.end_row = int(
            np.ceil(
                (st.session_state.start_row + len(st.session_state.allrows_correct)) / 2
            )
            * 2
        )
        if st.session_state.quality > 0.7:
            ...
            if len(st.session_state.methods) == 1:  # Single method
                ...
                if (
                    st.session_state.method_title.rsplit(" ")[0] == "Stedman"
                    or st.session_state.method_title.rsplit(" ")[0] == "Erin"
                ):
                    st.session_state.method_title = (
                        st.session_state.method_title.rsplit(" ")[0]
                        + " "
                        + st.session_state.method_title.rsplit(" ")[1]
                    )
                    st.session_state.lead_length = 12
                else:
                    st.session_state.lead_length = 4 * int(
                        st.session_state.hunt_types[0][1] + 1
                    )

            else:  # Spliced methods
                st.session_state.stages = [
                    name.rsplit(" ", 1)[-1]
                    for name in np.array(st.session_state.methods)[:, 0]
                ]
                st.session_state.stagetypes = [
                    name.rsplit(" ")[-2] + " " + name.rsplit(" ")[-1]
                    for name in np.array(st.session_state.methods)[:, 0]
                ]
                if len(set(st.session_state.stagetypes)) == 1:
                    st.session_state.method_title = (
                        "Spliced " + st.session_state.stagetypes[0]
                    )
                elif len(set(st.session_state.stages)) == 1:
                    st.session_state.method_title = (
                        "Spliced " + st.session_state.stages[0]
                    )
                else:
                    st.session_state.method_title = "Spliced"
                st.session_state.lead_length = 2 * int(
                    st.session_state.hunt_types[0][1] + 1
                )
            if st.session_state.quality > 0.85:
                st.method_message.write(
                    "**Method(s) detected: "
                    + str(st.session_state.nchanges)
                    + " "
                    + st.session_state.method_title
                    + "**"
                )
            ...
            st.session_state.cached_nchanges[st.session_state.current_touch] = (
                st.session_state.nchanges
            )

            if (
                st.session_state.nchanges
                / int(len(st.session_state.raw_actuals) / st.session_state.nbells)
                < 0.25
            ):
                st.session_state.start_row = 0
                st.session_state.end_row = int(
                    len(st.session_state.raw_actuals) / st.session_state.nbells
                )

        else:
            st.method_message.write(
                "**Probably a method but not entirely sure what...**"
            )
            st.session_state.cached_methods[st.session_state.current_touch] = (
                "Unknown Method"
            )
            st.session_state.cached_nchanges[st.session_state.current_touch] = int(
                len(st.session_state.raw_actuals) / st.session_state.nbells
            )
            st.session_state.method_flag = False
    ...
    if "Team Model" not in st.session_state.raw_data.columns.tolist():
        st.session_state.count_test = (
            st.session_state.rhythm_variation_time * st.session_state.nbells
        )
        st.session_state.gap_test = st.session_state.handstroke_gap_variation_time
        st.session_state.ideal_times = find_ideal_times_band(
            st.session_state.raw_data["Actual Time"],
            st.session_state.nbells,
            ncount=st.session_state.count_test,
            ngaps=st.session_state.gap_test,
            key=st.session_state.current_touch,
        )
        st.session_state.raw_data["Team Model"] = st.session_state.ideal_times
        st.session_state.existing_models.append("Team Model")

    st.session_state.ideal_times = find_ideal_times_rwp(
        st.session_state.raw_data["Actual Time"],
        st.session_state.nbells,
        key=st.session_state.current_touch,
    )
    ...
    if "Corrected Bells" not in st.session_state.raw_data.columns.tolist():
        ...
        st.session_state.corrected_order = st.session_state.raw_data[
            "Bell No"
        ].values.copy()  # The original order
        st.session_state.start_index = (
            st.session_state.start_row * st.session_state.nbells
        )
        st.session_state.corrected_order[
            st.session_state.start_index : st.session_state.start_index
            + np.size(st.session_state.allrows_correct)
        ] = np.ravel(st.session_state.allrows_correct)
    ...
    if "Metronomic Model" not in st.session_state.raw_data.columns.tolist():
        ...
        def find_metronomic(raw_data):
            ...
            for row in range(nrows):
                actual = np.array(
                    st.session_state.raw_actuals[
                        row
                        * st.session_state.nbells : (row + 1)
                        * st.session_state.nbells
                    ]
                )
                start = np.min(actual)
    ...
    if "Individual Model" not in st.session_state.raw_data.columns.tolist():
        ...
        st.session_state.gap_test = st.session_state.handstroke_gap_variation_time
        st.session_state.ideal_times = find_ideal_times(
            st.session_state.raw_data["Actual Time"],
            st.session_state.nbells,
            ncount=st.session_state.count_test,
            ngaps=st.session_state.gap_test,
    ...
    if len(st.session_state.existing_models) > 0:
        ...
        with st.expander("Change Statistical Options"):
            ...
            st.session_state.selection = st.selectbox(
                "Select striking model:",
                options=st.session_state.existing_models,
                index=st.session_state.existing_models.index("Team Model"),
            )  # Can set default for this later?
            # st.write(raw_data["Actual Time"][0:100:12])
            st.session_state.raw_target = np.array(
                st.session_state.raw_data[st.session_state.selection]
            )
            st.session_state.raw_bells = np.array(st.session_state.raw_data["Bell No"])
            st.session_state.correct_bells = np.array(
                st.session_state.raw_data["Corrected Bells"]
            )
            # Plot blue line
            st.session_state.nstrikes = len(st.session_state.raw_actuals)
            st.session_state.nrows = int(
                st.session_state.nstrikes // st.session_state.nbells
            )

            if st.session_state.selection == "Individual Model":
                st.session_state.rhythm_variation_time = st.slider(
                    "Rhythm variation time:",
                    min_value=2,
                    max_value=10,
                    value=4,
                    format="%d changes",
                ...
                st.session_state.handstroke_gap_variation_time = st.slider(
                    "Handstroke gap variation time:",
                    min_value=4,
                    max_value=20,
                    value=6,
                    format="%d changes",
                    step=2,
                    key=100 + st.session_state.current_touch,
                )
                st.session_state.rhythm_variation_time = (
                    st.session_state.rhythm_variation_time
                )
                st.session_state.ideal_times = find_ideal_times(
                    st.session_state.raw_data["Actual Time"],
                    st.session_state.nbells,
                    ncount=st.session_state.rhythm_variation_time
                    * st.session_state.nbells,
                    ngaps=st.session_state.handstroke_gap_variation_time,
                )
                if "Individual Model" not in st.session_state.raw_data.columns.tolist():
                    st.session_state.existing_models.append("Individual Model")

                st.session_state.raw_data["Individual Model"] = (
                    st.session_state.ideal_times
                )

            if st.session_state.selection == "Team Model":

                rhythm_variation_time = st.slider(
                    "Rhythm variation time:",
                    min_value=2,
                    max_value=10,
                    value=4,
                ...
                st.session_state.handstroke_gap_variation_time = st.slider(
                    ...
                    format="%d changes",
                    step=2,
                    key=200 + st.session_state.current_touch,
                )
                st.session_state.rhythm_variation_time = rhythm_variation_time
                ideal_times = find_ideal_times_band(
                    st.session_state.raw_data["Actual Time"],
                    st.session_state.nbells,
                    ncount=st.session_state.rhythm_variation_time
                    * st.session_state.nbells,
            ...
            st.session_state.remove_mistakes = st.checkbox(
                "Remove presumed method mistakes from the stats?", value=True
            )
            st.session_state.remove_confidence = st.checkbox(
                "Remove not-confident strike times from the stats?", value=True
            )
            if st.session_state.method_flag:
                st.session_state.use_method_info = st.checkbox(
                    "Use presumed composition to identify correct times?", value=True
                )
            else:
                st.session_state.use_method_info = False
            st.session_state.min_include_change, st.session_state.max_include_change = (
                st.slider(
                    "For the stats, include changes in range:",
                    min_value=0,
                    max_value=st.session_state.nrows,
                    value=(st.session_state.start_row + 1, st.session_state.end_row),
                    format="%d",
                    step=2,
                    key=300 + st.session_state.current_touch,
                )
            )
        ...
        st.session_state.overall_quality = (
            1.0 - np.mean(Strike_Data.alldiags[2, 2, :]) / Strike_Data.cadence
        )

        k = 17.5
        x0 = 0.727
        shifted_quality = 1.0 / (
            1.0 + np.exp(-k * (st.session_state.overall_quality - x0))
        )
        st.message.write(
            "Standard deviation from ideal for this touch: %.1fms"
            % np.mean(Strike_Data.alldiags[2, 2, :])
        )
        st.message_2.write(
            "Overall striking quality: **%.2f%%**" % (100 * shifted_quality)
        )

        st.session_state.cached_score[st.session_state.current_touch] = (
            100 * shifted_quality
        )  # This updates the cache
        st.session_state.cached_ms[st.session_state.current_touch] = np.mean(
            Strike_Data.alldiags[2, 2, :]
        )

        with st.expander("View Plaintext Striking Report"):
            st.empty()
            obtain_striking_markdown(
                Strike_Data.alldiags,
                Strike_Data.time_errors,
                Strike_Data.lead_times,
                Strike_Data.cadence,
                Strike_Data.remove_mistakes,
            )

        with st.expander("View Grid/Blue Line"):
            st.empty()
            st.session_state.min_plot_change, st.session_state.max_plot_change = (
                st.slider(
                    "View changes in range:",
                    min_value=0,
                    max_value=st.session_state.nrows,
                    value=(
                        st.session_state.start_row,
                        min(300, st.session_state.end_row),
                    ),
                    format="%d",
                    step=2,
                    key=400 + st.session_state.current_touch,
                )
            )
            st.session_state.view_numbers = st.checkbox(
                "View Bell Numbers", value=False
            )
            st.session_state.options = [
                "Bell %d" % bell for bell in range(1, st.session_state.nbells + 1)
            ]
            st.session_state.highlight_bells = st.pills(
                "Highlight Bells", st.session_state.options, selection_mode="multi"
            )
            plot_blue_line(
                st.session_state.raw_target,
                st.session_state.raw_actuals,
                Strike_Data.raw_bells,
                st.session_state.nbells,
                st.session_state.lead_length,
                st.session_state.min_plot_change,
                st.session_state.max_plot_change,
                st.session_state.highlight_bells,
                view_numbers=st.session_state.view_numbers,
            )
        ...
        with st.expander("View Error Bar Charts"):
            st.empty()
            plot_bar_charts(
                Strike_Data.alldiags, Strike_Data.nbells, st.session_state.titles
            )

        if st.session_state.calling_html is not None:
            with st.expander(
                "View Call Positions"
            ):  # Only if a single method and not a plain course -- need checks for this
        ...
        with st.expander("View Bell Errors Through Time"):
            ...
            st.session_state.min_plot_change, st.session_state.max_plot_change = (
                st.slider(
                    ...
                    min_value=0,
                    max_value=st.session_state.nrows,
                    value=(0, st.session_state.nrows),
                    format="%d",
                    step=2,
                    key=500 + st.session_state.current_touch,
                )
            )

            st.session_state.absvalues = st.radio(
                "Use absolute values?", ["Absolute Error", "Relative Error"]
            )
            st.session_state.options = ["Average"] + [
                "Bell %d" % bell for bell in range(1, st.session_state.nbells + 1)
            ]
            st.session_state.highlight_bells = st.pills(
                "Plot Bells",
                ...
                key=600 + st.session_state.current_touch,
            )
            ...
            if len(st.session_state.highlight_bells) == 1:
                st.session_state.strokes_plot = st.pills(
                    "Select Strokes",
                    strokes,
                    default="Both Strokes",
                    selection_mode="multi",
                    key=700 + st.session_state.current_touch,
                )
                ...
                st.session_state.strokes_plot = st.pills(
                    ...
                    key=800 + st.session_state.current_touch,
                )
            ...
            if (
                len(st.session_state.highlight_bells) > 0
                and st.session_state.strokes_plot is not None
            ):
                plot_errors_time(
                    Strike_Data.time_errors,
                    st.session_state.min_plot_change,
                    st.session_state.max_plot_change,
                    st.session_state.absvalues,
                    st.session_state.highlight_bells,
                    st.session_state.strokes_plot,
                    st.session_state.smooth,
                )

        with st.expander("View Histograms"):
            st.empty()
            st.session_state.x_range = st.slider(
                "Histogram x range:",
                min_value=50,
                max_value=250,
                value=160,
            ...
            st.session_state.nbins_default = min(
                100, max(int(len(Strike_Data.errors) / 2.5), 10)
            )
            st.session_state.nbins = st.slider(
                "Number of histogram bins",
                min_value=10,
                max_value=100,
                value=st.session_state.nbins_default,
                format="%d",
                step=1,
            )
            plot_histograms(
                ...
                st.session_state.nbins,
                st.session_state.nbells,
                Strike_Data.raw_bells,
                Strike_Data.correct_bells,
                Strike_Data.min_include_change,
                Strike_Data.max_include_change,
                Strike_Data.use_method_info,
                Strike_Data.remove_mistakes,
                Strike_Data.cadence,
                Strike_Data.raw_actuals,
                Strike_Data.raw_target,
        ...
        if st.session_state.current_touch > -1 and len(st.session_state.raw_titles) > 0:
            st.download_button(
                "Download analysis to device as .csv",
                csv,
                file_name=st.session_state.raw_titles[st.session_state.current_touch]
                + ".csv",
                mime="text/csv",
            )
