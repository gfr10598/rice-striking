...
def sort_current_collection(saved_index_list, oldfirst=False):
    ...
    for i in range(len(saved_index_list)):
        date_objects.append(
            datetime.strptime(saved_index_list[i, 5], "%m/%d/%Y, %H:%M:%S")
        )
    if oldfirst:
        sorted_list = np.array(
            [val for _, val in sorted(zip(date_objects, list_array), reverse=True)]
        )
    else:
        sorted_list = np.array(
            [val for _, val in sorted(zip(date_objects, list_array), reverse=False)]
        )
    np.savetxt(
        "./saved_touches/%s/index.csv" % st.session_state.current_collection_name,
        np.array(sorted_list, dtype=str),
        fmt="%s",
        delimiter=";",
    )
    st.rerun()
...
def add_new_folder(name):
    ...
    np.savetxt(
        "./saved_touches/%s/index.csv" % name,
        np.array([[" ", " ", " ", " ", " ", " ", " ", " "]], dtype="str"),
        fmt="%s",
        delimiter=";",
    )
...
if (
    "selected_library_touches" not in st.session_state
):  # List in index space of the currently-selected touches
...
if len(touch_titles) > 0:
    selection = st.pills(
        "Currently loaded touches:", touch_titles, default=touch_titles[-1]
    )
    ...
    st.write(
        "No touches are currently loaded. Load them from the library, upload a .csv from your"
        " device or from analysing a recording."
    )
    uploaded_files = st.file_uploader(
        "Upload file:",
        accept_multiple_files=True,
...
with cols[0]:
    if st.button(
        "Open an existing collection", disabled=st.session_state.collection_status == 2
    ):
...
with cols[1]:
    if st.button(
        "Create a new collection", disabled=st.session_state.collection_status == 1
    ):
...
if st.session_state.collection_status == 2:
    selected_name = st.text_input(
        "Enter existing collection name:", key=st.session_state.input_key + 1000
    )
...
if st.session_state.collection_status == 1:
    st.write(
        "Choose a name (a single word) for the new collection. If you'd like it not to be found by"
        " anyone else, choose something unguessable."
    )
    new_collection_name = st.text_input(
        "New collection name (no capitals, spaces or special characters)",
        key=st.session_state.input_key,
    )
    if new_collection_name is not None:
        new_collection_name = re.sub(r"[^\w\-]", "_", new_collection_name)
        new_collection_name = re.sub(
            r"[A-Z]", lambda m: m.group(0).lower(), new_collection_name
        )

    if new_collection_name is not None:
        if (
            new_collection_name not in existing_names
            and new_collection_name
            and len(new_collection_name) > 0
        ):
            st.write('"%s" is valid and unused. Good good.' % new_collection_name)
            st.write(
                "**Remember this name -- it can't be recovered if you forget it!**"
            )
            ...
            st.write(
                "This collection already exists... Click the relevant button above to open it."
            )
            st.stop()
    ...
    st.write(
        "Share this collection with the url"
        " https://brenda.oekrice.com/Analyse_Striking?collection=%s"
        % st.session_state.current_collection_name
    )
else:
    st.stop()

# I think the above logic is all sound. It's a bit tricky. Now to present a list of touches in said
#  collection.
saved_index_list: np.ndarray = np.zeros(shape=(0, 8), dtype="str")
...
with st.expander("Add/remove touches and change touch metadata"):
    ...
    if (
        st.session_state.addition_mode < 0
        and st.session_state.subtraction_mode < 0
        and st.session_state.rename_mode < 0
    ):
        ...
        with cols[2]:
            if st.button("Sort (newest first)", disabled=disabled_flag):
                ...
                st.rerun()
    # Listitems will be ID (random string), readable ID (nonunique), nchanges, method(s), tower,
    #  datetime. All as strings.
    ...
    if st.session_state.addition_mode > 0:
        ...
        if len(touch_titles) == 0:
            st.write(
                "No touches are loaded. Either analyse a recording, upload a csv. or load from"
                " another collection."
            )
            st.stop()
        else:
            ...
            if saved_index_list[0][0] != " ":
                ...
                current_entries = np.array([], dtype=str)
            select_list = []
            ...
            st.write(
                "If the touch you want isn't in this list, either analyse the recording or upload a"
                " .csv"
            )
                ...
                for ti in select_list:
                    ...
                    if len(methods) > 0:
                        ...
                        if quality > 0.7:
                            ...
                            st.session_state.cached_nchanges[ti] = int(
                                len(st.session_state.cached_rawdata[ti]["Bell No"])
                                / nbells
                            )
                    else:
                        st.session_state.cached_methods[ti] = "Rounds and/or Calls"
                        st.session_state.cached_nchanges[ti] = int(
                            len(st.session_state.cached_rawdata[ti]["Bell No"]) / nbells
                        )

                    if st.session_state.cached_touch_id[ti] not in current_entries:

                        new_list_entry = [
                            st.session_state.cached_touch_id[ti],
                            st.session_state.cached_read_id[ti],
                            st.session_state.cached_nchanges[ti],
                            st.session_state.cached_methods[ti],
                            st.session_state.cached_tower[ti],
                            st.session_state.cached_datetime[ti],
                            st.session_state.cached_score[ti],
                            st.session_state.cached_ms[ti],
                        ]
    ...
    if st.session_state.rename_mode > 0:
        ...
        new_name = st.text_input(
            "Enter new name",
            value=saved_index_list[select_options.index(rename_touch), 1],
        )

        # Change time
        date_object = datetime.strptime(
            saved_index_list[select_options.index(rename_touch), 5],
            "%m/%d/%Y, %H:%M:%S",
        )
        new_proper_time = st.text_input(
            "Enter time", date_object.strftime("%d-%m-%Y %H:%M:%S")
        )
        try:
            new_date_object = datetime.strptime(new_proper_time, "%d-%m-%Y %H:%M:%S")
        except ValueError:
            st.error(
                "Incorrect date format, or incorrect date. Try again. Format is DD-MM-YYY HH:MM:SS"
            )
            st.stop()
        ...
        new_tower_name = st.text_input(
            "Enter new tower name",
            saved_index_list[select_options.index(rename_touch), 4],
        )
        ...
        with cols[0]:
            if st.button("Apply Changes"):
                ...
                saved_index_list[select_options.index(rename_touch), 5] = (
                    new_date_object.strftime("%m/%d/%Y, %H:%M:%S")
                )
                change_rename_mode()
                np.savetxt(
                    "./saved_touches/%s/index.csv"
                    % st.session_state.current_collection_name,
                    np.array(saved_index_list, dtype=str),
                    fmt="%s",
                    delimiter=";",
                )
                st.rerun()
        ...
        with cols[0]:
            if len(new_name) < 1:
                ...
                if new_name in existing_names:
                    ...
                    os.system(
                        "mv ./saved_touches/%s ./saved_touches/%s"
