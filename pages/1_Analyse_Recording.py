...
from listen_main_functions import (
    establish_initial_rhythm,
    do_reinforcement,
    find_final_strikes,
    save_strikes,
    filter_final_strikes,
)
...
if "use_existing_freqs" not in st.session_state:
    st.session_state.use_existing_freqs = (
        -1
    )  # Positive if do want to use existing frequencies. Negative if not.
if "already_saved" not in st.session_state:
    st.session_state.already_saved = False
if "good_frequencies_selected" not in st.session_state:
    st.session_state.good_frequencies_selected = (
        False  # Decent frequencies are selected (either preloaded or calculated now)
    )
...
if "collection_status" not in st.session_state:
    st.session_state.collection_status = (
        -1
    )  # -1 for nothing, 0 for opening an existing one and 1 for creating a new one
...
def determine_collection_from_url(existing_names):
    if "collection" in st.query_params.keys():
        ...
        collection_name = re.sub(
            r"[A-Z]", lambda m: m.group(0).lower(), collection_name
        )
        ...
        for touch_info in saved_index_list:
            ...
            if touch_info[0] not in st.session_state.cached_touch_id:
                ...
                raw_data = pd.read_csv(
                    "./saved_touches/%s/%s.csv"
                    % (st.session_state.current_collection_name, touch_info[0])
                )

                st.session_state.cached_data.append(
                    [
                        touch_info[4],
                        int(len(raw_data) / np.max(raw_data["Bell No"])),
                        touch_info[1],
                    ]
                )
                ...
                st.session_state.cached_data[cache_index] = [
                    touch_info[4],
                    int(len(raw_data) / np.max(raw_data["Bell No"])),
                    touch_info[1],
                ]
    ...
    if "current_collection_name" in st.session_state:
        if st.session_state.current_collection_name is not None:
            st.query_params.from_dict(
                {"collection": [st.session_state.current_collection_name]}
            )
    return
...
if url_collection is not None:
    ...
    if (
        os.path.getsize(
            "./saved_touches/%s/index.csv" % st.session_state.current_collection_name
        )
        > 0
    ):
        saved_index_list = np.loadtxt(
            "./saved_touches/%s/index.csv" % st.session_state.current_collection_name,
            delimiter=";",
            dtype=str,
        )
    else:
        saved_index_list = np.array(
            [[" ", " ", " ", " ", " ", " ", " ", " "]], dtype="str"
        )
...
st.session_state.default_index = st.session_state.tower_names.index(
    "Brancepeth, S Brandon"
)
...
if st.session_state.tower_name is not None:
    if st.session_state.tower_name != "Unknown Tower":
        ...
        st.session_state.index = st.session_state.tower_names.index(
            st.session_state.tower_name
        )
        # st.session_state.nominals_confirmed = True

if st.session_state.testing_mode:
    st.session_state.tower_name = st.selectbox(
        "Select a tower...",
        st.session_state.tower_names,
        index=st.session_state.default_index,
        key=None,
        placeholder="Choose a tower",
        label_visibility="visible",
        on_change=reset_nominals,
    )
...
if st.session_state.tower_name:
    ...
    st.session_state.nominals_confirmed = False
    st.write(
        "If you can't find the required tower in the list, try to find a similarly-tuned ring (look"
        " for the nominal frequencies) on Dove's guide and that may work -- but please don't save"
...
if st.session_state.tower_selected:
    ...
    st.session_state.tower_id = st.session_state.nominal_data["Tower ID"][
        selected_index
    ]

    st.session_state.bell_options = list(st.session_state.nominal_data.columns)[3:]
    # Determine the number of valid bells
    st.session_state.bell_names = []
    st.session_state.bell_nominals = []
    ...
    for option in st.session_state.bell_options:
        if float(st.session_state.nominal_data.loc[selected_index][option]) > 0:
            ...
            if option.isnumeric():
                st.session_state.nbells_max = max(
                    st.session_state.nbells_max, int(option)
                )

    st.write(
        "Ring of ",
        str(st.session_state.nbells_max),
        "with",
        str(len(st.session_state.bell_names)),
        "bells to choose from:",
    )

    st.session_state.nrows = len(st.session_state.bell_names) // 11 + 1
    st.session_state.per_row = (
        int((len(st.session_state.bell_names) - 1e-6) // st.session_state.nrows) + 1
    )
    st.session_state.mincheck = len(st.session_state.bell_names) - 1
    st.session_state.maxcheck = 0

    st.session_state.nbells_save = 0
    st.session_state.max_bell = 0

    if st.session_state.testing_mode:
        st.session_state.nominals_confirmed = False

        st.session_state.nbells_select = int(
            input_matrix[st.session_state.test_counter][1]
        )
        for row in range(st.session_state.nrows):
            ...
            st.session_state.end = min(
                len(st.session_state.bell_names), (row + 1) * st.session_state.per_row
            )
            st.session_state.cols = st.columns(
            ...
            for i in range(st.session_state.start, st.session_state.end):
                ...
                if st.session_state.bell_names[i].isnumeric():
                    ...
                    st.session_state.bell_nominals.append(
                        float(
                            st.session_state.nominal_data.loc[
                                selected_index, st.session_state.bell_names[i]
                            ]
                        )
                    )

        st.session_state.mincheck = (
            st.session_state.maxcheck - st.session_state.nbells_select + 1
        )
        st.session_state.max_bell = st.session_state.maxcheck + 1
        st.session_state.bell_nominals = st.session_state.bell_nominals[
            -st.session_state.nbells_select :
        ]
    else:
                ...
                with cols[i % st.session_state.per_row]:
                    if len(st.session_state.checked) > 0:
                        if st.session_state.bell_names[i] in st.session_state.checked:
                            checked = st.checkbox(
                                st.session_state.bell_names[i],
                                value=True,
                                on_change=reset_nominals,
                                key=i + st.session_state.tower_id * 1000,
                            )
                            if st.session_state.bell_names[i].isnumeric():
                                st.session_state.max_bell = max(
                                    st.session_state.max_bell,
                                    int(st.session_state.bell_names[i]),
                                )
                        else:
                            checked = st.checkbox(
                                st.session_state.bell_names[i],
                    ...
                    if checked:
                        st.session_state.allchecked.append(
                            st.session_state.bell_names[i]
                        )
                        st.session_state.mincheck = min(i, st.session_state.mincheck)
                        st.session_state.maxcheck = max(i, st.session_state.maxcheck)
    ...
    st.session_state.bell_nominals = sorted(
        st.session_state.bell_nominals, reverse=True
    )

    st.write(
        str(len(st.session_state.bell_nominals)),
        "bells selected, with (editable) nominal frequencies in Hz",
    )
    ...
    if st.session_state.nominals_confirmed:
        ...
        freq_df = pd.DataFrame(
            data=np.array(st.session_state.bell_nominals)[np.newaxis, :],
            columns=[
                "Bell %d" % (ri + 1)
                for ri in range(len(st.session_state.bell_nominals))
            ],
        )
    else:
    ...
    edited_nominals = np.array(
        st.data_editor(freq_df, hide_index=True, on_change=reset_nominals)
    )[0].tolist()
    ...
    if st.session_state.testing_mode:
        ...
        if st.button(
            "Confirm Tower and Frequencies",
            disabled=st.session_state.nominals_confirmed,
        ):
            st.session_state.nominals_confirmed = True
            st.session_state.bell_nominals = edited_nominals
            st.rerun()
...
if st.session_state.tower_selected and st.session_state.nominals_confirmed:
    ...
    st.session_state.freq_root = "%05d_%02d_%02d" % (
        st.session_state.tower_id,
        st.session_state.nbells_save,
        st.session_state.max_bell,
    )
    ...
    best_freq_quality = 0.0
    if len(allquals) > 0:
        best_freq_quality = np.max(allquals)

    if frequency_counter == 1:
        st.write(
            "Found %d existing frequency profile which matches the selected bells:"
            % frequency_counter
        )
        # st.write('Choose existing profile or make a new one (can change your mind later):.')
        allstrings = [
            "Make new profile",
            ":%s[Profile 1: %.1f%% match]" % (allcs[0], 100 * allquals[0]),
        ]
        if best_freq_quality > 0.97:
            options = st.radio(
                "Choose existing profile or make a new one (can change your mind later):",
                allstrings,
                on_change=stop_analysis,
                index=1,
            )
        else:
            options = st.radio(
        ...
        st.write(
            "Found %d existing frequency profiles which match the selected bells..."
            % frequency_counter
        )
        # st.write('Choose existing profile or make a new one (can change your mind later):')
        ...
        for qi, qual in enumerate(allquals):
            allstrings.append(
                ":%s[Profile %d: %.1f%% match]" % (allcs[qi], qi + 1, 100 * qual)
            )
        options = st.radio(
            "Choose existing profile or make a new one (can change your mind later):",
            allstrings,
        ...
        st.write(
            "No existing frequency profiles which match these bells -- will need to create one."
        )
    ...
    if options == "Make new profile":
        ...
        st.session_state.existing_filename = st.session_state.freq_root + "_%03d" % (
            st.session_state.use_existing_freqs
        )
    ...
    if st.session_state.testing_mode:
        ...
        st.session_state.input_option = st.pills(
            label="Upload a recording or record something directly:",
            options=["Upload", "Record"],
            default="Upload",
            on_change=reset_file,
        )
        if st.session_state.input_option != "Record":
            input_file = st.file_uploader(
                "Upload recording of ringing for analysis",
            ...
            if input_file is not None:
                ...
                st.session_state.tower_short = st.session_state.tower_name.rsplit(" ")[
                    0
                ][:-1]
                input_file.name = "%s_record_%04d.wav" % (
                    st.session_state.tower_short,
    ...
    if (st.session_state.raw_file is not None) and (
        st.session_state.audio_signal is not None
    ):
        st.audio(st.session_state.raw_file)
    if (
        st.session_state.input_option == "Record"
        and st.session_state.raw_file is not None
    ):
        st.download_button(
            "Download this recording to device",
            st.session_state.raw_file,
            file_name=st.session_state.raw_file.name,
        )

    if input_option == "Upload":
        if st.session_state.trimmed_signal is not None:
            st.write(
                'File "%s" read in successfully.' % st.session_state.audio_filename
            )
            st.write(
                "Trimmed recording length: %d seconds."
                % (len(st.session_state.trimmed_signal) / st.session_state.fs)
            )
            ...
            st.write(
                "Imported recording length: %d seconds."
                % (len(st.session_state.audio_signal) / st.session_state.fs)
            )
    else:
        if st.session_state.trimmed_signal is not None:
...
if (
    st.session_state.nominals_confirmed
    and st.session_state.tower_selected
    and (
        st.session_state.audio_signal is not None
        or st.session_state.trimmed_signal is not None
    )
):
    ...
    st.session_state.overall_tmin, st.session_state.overall_tmax = st.slider(
        "Trim recording for use overall (remove silence before ringing if possible):",
        min_value=0.0,
        max_value=0.0,
        value=(0.0, st.session_state.tmax),
        step=1.0,
        format="%ds",
        disabled=False,
    )
    ...
    if st.session_state.use_existing_freqs < 0:
        st.session_state.reinforce_tmax = st.slider(
            "Length of recording used for frequency analysis -- longer is slower but more accurate"
            " (but don't include bad ringing!):",
            min_value=45.0,
            max_value=min(120.0, st.session_state.tmax),
            step=1.0,
            value=(60.0),
            format="%ds",
        )
        st.session_state.nreinforces = int(
            st.slider(
                "Max number of frequency analysis steps -- may not need this many and will stop"
                " automatically if so:",
                min_value=2,
                max_value=15,
                value=10,
    ...
    if st.session_state.use_existing_freqs < 0:
        if len(st.session_state.bell_nominals) > 8:
            st.session_state.npicks_mode = st.checkbox(
                label="Run a more intense frequency analysis to improve accuracy (may be very slow"
                " but recommended on 10/12)",
                value=True,
            )
        else:
            st.session_state.npicks_mode = st.checkbox(
    ...
    st.session_state.Paras = parameters(
        np.array(st.session_state.bell_nominals),
        st.session_state.overall_tmin,
        st.session_state.overall_tmax,
        st.session_state.reinforce_tmax,
        st.session_state.nreinforces,
        st.session_state.npicks_mode,
    )
    ...
    if st.session_state.use_existing_freqs < 0:
        ...
        if st.session_state.testing_mode:
            ...
            if (
                st.session_state.reinforce_status == 0
                or st.session_state.reinforce_status == 2
            ):
                st.button(
                    "Find new frequency profiles and strike times",
                    on_click=change_reinforce,
                )
            else:
        ...
        if st.session_state.reinforce_status == 1:
            ...
            if (
                st.session_state.reinforce_frequency_data is not None
                and st.session_state.reinforce_frequency_data[2] > 0.005
            ):
                toprint = st.session_state.reinforce_frequency_data[2]
                c = find_colour(toprint)
                st.quality_log.write(
                    "Best yet frequency match: :%s[%.1f%%]" % (c, 100 * toprint)
                )
            else:
                st.quality_log.write(
                    "Best yet frequency match: :%s[%.1f%%]" % ("red", 0.0)
                )
            ...
            st.current_log.write(
                "Established initial rhythm using "
                + str(len(st.session_state.Data.strikes[0]))
                + " changes"
            )

            st.session_state.Data = do_reinforcement(
                st.session_state.Paras, st.session_state.Data
            )
            ...
            if st.session_state.reinforce_frequency_data is not None:
                if st.session_state.reinforce_frequency_data[2] > 0.85:
                    ...
                    st.main_log.write(
                        "**Frequency profiles not good enough to use... Apologies**"
                    )
                    st.session_state.reinforce_status = 0
            else:
                st.session_state.reinforce_status = 0
        ...
        if (
            st.session_state.reinforce_status == 2
        ):  # At least some frequency reinforcement has happened, print out some things to this end

            st.main_log.write("**Frequency analysis completed seemingly succesfully**")
            # Determine colours:
            colour_thresholds = [0.95, 0.98]
            ...
            st.quality_log.write(
                "Best frequency analysis quality = :%s[%.1f%%]" % (c, 100 * toprint)
            )

            if toprint < 0.95:
                st.current_log.write(
                    "This might not be good enough to provide anything useful. Suggest using a nicer touch or doing more intense analysis. But may as well try if not..."
                )
            elif toprint < 0.975:
                st.current_log.write(
                    "Not perfect but it'll probably do. If accuracy is important I'd suggest finding a nicer recording to learn the frequencies, or run a more intense analysis."
                )
            else:
                st.current_log.write(
                    "That should be fine to detect everything reasonably well."
                )
...
if (
    st.session_state.reinforce_status == 2 and st.session_state.use_existing_freqs < 0
) or st.session_state.use_existing_freqs >= 0:
...
if (
    st.session_state.good_frequencies_selected
    and st.session_state.trimmed_signal is not None
    and st.session_state.nominals_confirmed
    or (st.session_state.analysis_status == 2)
):
    ...
    if (
        ...
        if (
            st.session_state.use_existing_freqs < 0
            and st.session_state.analysis_status != 2
        ):
            st.empty().write(
                "New frequency profile calculated. Ready to find strike times."
            )
        elif st.session_state.analysis_status != 2:
            st.empty().write(
                "Existing frequency profile loaded. Ready to find strike times."
            )
        else:
            st.empty().write(
                "Strike times already found but you're welcome to try again if it's gone wrong."
            )
    ...
    if st.session_state.analysis_status == 1:
        ...
        if st.session_state.use_existing_freqs < 0:
            ...
            st.session_state.final_freqprobs = (
                st.session_state.reinforce_frequency_profile
            )
        else:
            st.session_state.final_freqs = np.load(
                "./frequency_data/" + st.session_state.existing_filename + "_freqs.npy"
            )
            st.session_state.final_freqprobs = np.load(
                "./frequency_data/"
        ...
        bellconfs_individual = np.mean(
            np.array(st.session_state.allcerts)[1:, :], axis=0
        )
        st.session_state.reinforce_frequency_data = np.array(
            [
                st.session_state.Paras.dt,
                st.session_state.Paras.fcut_length,
                np.mean(np.array(st.session_state.allcerts)[1:]),
                np.min(np.array(st.session_state.allcerts)[1:]),
            ]
        )
        st.session_state.reinforce_frequency_data = np.concatenate(
            (st.session_state.reinforce_frequency_data, bellconfs_individual)
        )
    ...
    if st.session_state.analysis_status == 2:
        ...
        st.analysis_sublog2.write(
            "**%d rows found with average confidence :%s[%.1f%%]**"
            % (
                len(st.session_state.allstrikes[0]),
                c,
                100 * np.mean(st.session_state.allcerts),
            )
        )
        ...
        for row in range(len(st.session_state.allstrikes[0])):
            order = np.array(
                [
                    val
                    for _, val in sorted(
                        zip(st.session_state.allstrikes[:, row], yvalues), reverse=False
                    )
                ]
            )
            allstrikes = allstrikes + sorted(
                (st.session_state.allstrikes[:, row]).tolist()
            )
        ...
        methods, hunt_types, calls, start_row, end_row, allrows_correct, quality = (
            find_method_things(allbells)
        )
        ...
        if (
            st.session_state.use_existing_freqs == -1
            and not st.session_state.already_saved
            and goodenough
            and freq_filename is not None
            and st.session_state.reinforce_frequency_data is not None
        ):
            ...
            if np.mean(st.session_state.allcerts) > best_freq_quality:
                if np.mean(st.session_state.allcerts) > 0.98:
                    ...
                    np.mean(st.session_state.allcerts) > 0.95
                    and quality > 0.95
                    and len(allrows_correct) > 60
                    and len(methods) > 0
                ):
                    save_automatically = True
            ...
            st.save_option.write(
                "Save these bell frequency profiles for future use? Only do so if you're confident this analysis is accurate and there don't already exist better profiles."
            )

            # Redefine to be based on an entire touch given that's what matters
            st.session_state.reinforce_frequency_data[2] = np.mean(
                st.session_state.allcerts
            )
            if save_automatically:
                np.save(
                    "%s%s_freqs.npy" % ("./frequency_data/", freq_filename),
                    st.session_state.reinforce_test_frequencies,
                )
                np.save(
                    "%s%s_freqprobs.npy" % ("./frequency_data/", freq_filename),
                    st.session_state.reinforce_frequency_profile,
                )
                np.save(
                    "%s%s_freq_quality.npy" % ("./frequency_data/", freq_filename),
                    st.session_state.reinforce_frequency_data,
                )
                st.session_state.already_saved = True
                if not st.session_state.testing_mode:
                    st.rerun()

            if st.save_button.button(
                "Save bell frequency profiles", disabled=st.session_state.already_saved
            ):
                np.save(
                    "%s%s_freqs.npy" % ("./frequency_data/", freq_filename),
                    st.session_state.reinforce_test_frequencies,
        ...
        if len(methods) > 0:
            if quality > 0.7:
                ...
                if len(methods) == 1:  # Single method
                    ...
                    if (
                        method_title.rsplit(" ")[0] == "Stedman"
                        or method_title.rsplit(" ")[0] == "Erin"
                    ):
                        method_title = (
                            method_title.rsplit(" ")[0]
                            + " "
                            + method_title.rsplit(" ")[1]
                        )

                else:  # Spliced methods
                    stages = [
                        name.rsplit(" ", 1)[-1] for name in np.array(methods)[:, 0]
                    ]
                    stagetypes = [
                        name.rsplit(" ")[-2] + " " + name.rsplit(" ")[-1]
                        for name in np.array(methods)[:, 0]
                    ]
        ...
        if not st.session_state.incache:
            if (
                st.session_state.audio_filename in cache_names
            ):  # This does the check. Seems reasonable
                ...
                if st.session_state.handstroke_first:
                    ...
                    st.session_state.cached_strikes[update_index] = (
                        st.session_state.allstrikes
                    )
                    st.session_state.cached_certs[update_index] = (
                        st.session_state.allcerts
                    )
                    st.session_state.cached_data[update_index] = [
                        st.session_state.tower_name,
                        len(st.session_state.allstrikes[0]),
                        st.session_state.audio_filename,
                    ]
                    ...
                    st.session_state.cached_strikes[update_index] = (
                        st.session_state.allstrikes[:, 1:]
                    )
                    st.session_state.cached_certs[update_index] = (
                        st.session_state.allcerts[:, 1:]
                    )
                    st.session_state.cached_data[update_index] = [
                        st.session_state.tower_name,
                        len(st.session_state.allstrikes[0]),
                    ...
                    st.session_state.touch_length = (
                        len(st.session_state.allstrikes[0]) - 1
                    )
                ...
                if st.session_state.handstroke_first:
                    ...
                    st.session_state.cached_touch_id.append(
                        "".join(
                            random.choices(string.ascii_letters + string.digits, k=10)
                        )
                    )
                    st.session_state.cached_read_id.append(
                        st.session_state.audio_filename
                    )
                    ...
                    st.session_state.cached_datetime.append(
            ...
            st.page_link(
                "pages/2_Analyse_Striking.py", label=":blue[Analyse this striking]"
            )

        if len(st.session_state.cached_strikes) == 1:
            st.write(
                "%d set of striking data currently in the cache - view using the tab on the left or the link above"
                % len(st.session_state.cached_strikes)
            )
        elif len(st.session_state.cached_strikes) > 1:
            st.write(
                "%d sets of striking data currently in the cache - view using the tab on the left or the link above"
                % len(st.session_state.cached_strikes)
            )

        # Give options to save to the cache (so this works on the analysis page) or to download as
        #  a csv
        # Create strike data in the right format (like the Strikeometer)
        # striking_df.attrs = {"Tower Name": st.session_state.tower_name, "Touch Length":
        #  st.session_state.touch_length, "File Name": st.session_state.audio_filename}
        @st.cache_data(ttl=300)
        ...
        st.download_button(
            "Download raw timing data to device as .csv",
            csv,
            file_name=st.session_state.audio_filename + ".csv",
            mime="text/csv",
        )
        ...
        if st.session_state.testing_mode:
            ...
            print(
                "Confidence: ", "%.1f %% " % (100 * np.mean(st.session_state.allcerts))
            )
            allstrikes_save = []

            for bell in range(len(st.session_state.allstrikes)):
                diff1s = (
                    st.session_state.allstrikes[0][2::2]
                    - st.session_state.allstrikes[0][0:-2:2]
                )
                diff2s = (
                    st.session_state.allstrikes[0][3::2]
                    - st.session_state.allstrikes[0][1:-2:2]
                )
            for row in range(len(st.session_state.allstrikes[0])):
                allstrikes_save = allstrikes_save + sorted(
                    (st.session_state.allstrikes[:, row]).tolist()
                )
            allstrikes_save = np.array(allstrikes_save)
            ideal_times = find_ideal_times_rwp(
                allstrikes_save, st.session_state.Paras.nbells, key=0
            )
