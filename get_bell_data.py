...
As a pandas dataframe. First column tower name, second dove id and the rest (up to 16) as the rest
 of the columns
"""
...
if True:
    ...
    data = pd.read_csv(
        "https://dove.cccbr.org.uk/bells.csv?bells=all&ring_type=english"
    )
...
for bellcount in range(len(df) - 1):
    ...
    if df["Tower ID"][bellcount + 1] != tower_id:  # New tower
        if fine:
            allrows_list.append(row_data)  # Append last one
...
def tower_alias(df_new, tower_id, alias_id, tower_name):
    # Used on a bespoke basis when the Dove data is incomplete. Will assign the data from
    #  'alias name' to 'tower name'
...
df_new = tower_alias(
    ...
    15838,
    14094,
    "Sunderland, Bishopwearmouth, Minster Ch of S Michael & All Angels & S Benedict Biscop",
)
