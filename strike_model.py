...
from scipy.optimize import minimize_scalar, curve_fit, OptimizeWarning  # type: ignore
import warnings
...
def find_ideal_times(alltimes, nbells, ncount=24, ngaps=6, key=-1):
    ...
    for strike in range(len(all_ideals)):
        ...
        if strike == 0 or strike == 1:  # First strikes are naturally perfect
            ...
            strike < nbells
        ):  # Not enough preceding data, assume a linear interpolation from preceding
            data = np.array(alltimes[:strike])
            data = adjust_times(
                data, row_number, n_adjust, row_position
