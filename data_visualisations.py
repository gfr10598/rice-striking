...
# This file contains all the stuff which is currently cluttering up the second page, which has got
#  a bit out of hand
...
def calculate_stats(Strike_Data):
    ...
    diffs = (
        np.array(Strike_Data.raw_actuals)[1:] - np.array(Strike_Data.raw_actuals)[:-1]
    )
    Strike_Data.cadence = (
        np.mean(diffs) * (2 * Strike_Data.nbells) / (2 * Strike_Data.nbells + 1)
    )  # Mean interbell gap (ish)

    Strike_Data.time_errors = np.zeros(
        (
            Strike_Data.nbells,
            int(len(np.array(Strike_Data.raw_actuals)) / Strike_Data.nbells),
        )
    )  # Errors through time for the whole touch
        ...
        for bell in range(1, Strike_Data.nbells + 1):  # nbells):
            ...
            lead_bell = [
                (
                    ...
                    if val
                    in np.arange(
                        0, len(bellstrikes) * Strike_Data.nbells, Strike_Data.nbells
                    )
                    else False
                )
                for val in bellstrikes
            ]
            Strike_Data.lead_times[bell - 1] = lead_bell

            if Strike_Data.use_method_info:
                Strike_Data.errors = np.array(
                    Strike_Data.raw_actuals[bellstrikes]
                    - Strike_Data.raw_target[targetstrikes]
                )
            else:
                Strike_Data.errors = np.array(
                    Strike_Data.raw_actuals[bellstrikes]
                    - Strike_Data.raw_target[bellstrikes]
                )
            ...
            bellstrikes = bellstrikes[
                bellstrikes / Strike_Data.nbells >= Strike_Data.min_include_change
            ]
            bellstrikes = bellstrikes[
                bellstrikes / Strike_Data.nbells <= Strike_Data.max_include_change
            ]
            ...
            # Attempt to remove outliers (presumably method mistakes, hawkear being silly or other
            #  spannering)
            ...
            if count > 2:
                # Diagnostics
                Strike_Data.alldiags[0, plot_id, bell - 1] = (
                    np.sum(Strike_Data.errors) / count
                )
                Strike_Data.alldiags[1, plot_id, bell - 1] = np.sqrt(
                    np.sum(
                        (Strike_Data.errors - np.sum(Strike_Data.errors) / count) ** 2
                    )
                    / count
                )
                Strike_Data.alldiags[2, plot_id, bell - 1] = np.sqrt(
                    np.sum(Strike_Data.errors**2) / count
                )
    return Strike_Data


# Do plain-text appraisal of the striking. Give the most accurate and most consistent bells, and
#  oddstruckness. Will add leading if that's a thing too
@st.cache_data(ttl=300)
def obtain_striking_markdown(
    alldiags, time_errors, lead_times, cadence, remove_mistakes
):
    ...
    if most_consistent == most_accurate:
        lines.append(
            "Bell %d was the most consistent and accurate <br>" % (most_consistent + 1)
        )
    else:
    ...
    def produce_strings(bell, lines, errors_all, errors_hand, errors_back, lead=False):
        ...
        if (
            overall_oddstrucknesses[bell][1] == overall_oddstrucknesses[bell][2]
        ) and overall_oddstrucknesses[bell][1] != 0:
            text = number_to_words(overall_oddstrucknesses[bell][1])
            if not lead:
                lines.append("Bell %d was generally %s. <br>" % (bell + 1, text))
            else:
                lines.append(
                    "Bell %d was generally %s leading. <br>" % (bell + 1, text)
                )
        else:
            text1 = None
            text2 = None
            ...
            if text1 is not None:
                if not lead:
                    lines.append(
                        "Bell %d was %s at handstroke <br>" % (bell + 1, text1)
                    )
                else:
                    lines.append(
                        "Bell %d was %s leading at handstroke<br>" % (bell + 1, text1)
                    )
            if text2 is not None:
                if not lead:
                    lines.append(
                        "Bell %d was %s at backstroke <br>" % (bell + 1, text2)
                    )
                else:
                    lines.append(
                        "Bell %d was %s leading at backstroke<br>" % (bell + 1, text2)
                    )
        return lines
    ...
    for bell in range(nbells):
        ...
        lines = produce_strings(
            bell, lines, errors_all, errors_hand, errors_back, lead=False
        )
        lines = produce_strings(
            bell, lines, leading_all, leading_hand, leading_back, lead=True
        )
            ...
            for stroke in strokes_plot:
                if bell == "Average":
                    ...
                    if stroke == "Backstrokes":
                        if mode == 2:
                            ax5.plot(
                                back_xs, back_avg_ys, c="green", label="Backstrokes"
                            )
                        else:
                            ax5.plot(
                                back_xs,
...
def plot_bar_charts(alldiags, nbells, titles):
    try:
        ...
        data_titles = [
            "Avg. Error (positive is slow, negative is quick)",
        ...
        for plot_id in range(3):
            ...
            for bell in range(1, nbells + 1):  # nbells):
                ...
                targetstrikes = targetstrikes[
                    targetstrikes / nbells >= min_include_change
                ]
                targetstrikes = targetstrikes[
                    targetstrikes / nbells <= max_include_change
                ]
                ...
                if use_method_info:
                    errors = np.array(
                        raw_actuals[bellstrikes] - raw_target[targetstrikes]
                    )
                else:
                    errors = np.array(
                        raw_actuals[bellstrikes] - raw_target[bellstrikes]
                    )

                # Attempt to remove outliers (presumably method mistakes, hawkear being silly or
                #  other spannering)
                maxlim = cadence * 0.75
                minlim = -cadence * 0.75
    ...
    try:
        ...
        for plot_id in range(3):
            ...
            for bell in range(1, nbells + 1):  # nbells):
                ...
                # Box plot data
                ax.boxplot(
                    bell_errors, positions=[bell], sym="x", widths=0.35, zorder=1
                )
...
def plot_blue_line(
    ...
    try:
        ...
        highlight_bells = [
            int(highlight_bells[val][5:]) for val in range(len(highlight_bells))
        ]
        ...
        for plot in range(nplotsk):
            ...
            for bell in range(1, nbells + 1):  # nbells):
                ...
                if bell in highlight_bells:
                    ...
                    bellstrikes = np.where(raw_bells == bell)[0]
                    for row in range(
                        plot * rows_per_plot + min_plot_change,
                        min((plot + 1) * rows_per_plot + min_plot_change + 1, maxrow),
                    ):
                        # Find linear position... Linear interpolate?
                        target_row = np.array(
                            raw_target_plot[row * nbells : (row + 1) * nbells]
                        )
                        if len(target_row) == nbells:
                            ys = np.arange(1, nbells + 1)
                            f = interpolate.interp1d(
                                target_row, ys, fill_value="extrapolate"
                            )
                    ...
                    if len(highlight_bells) > 0:
                        ...
                        if view_numbers:
                            ...
                            ax.plot(
                                points, changes, label=bell, c=cmap[(bell - 1) % 10]
                            )

                ax.plot(
                    (bell) * np.ones(len(points)),
                    changes,
                    c="black",
                    linewidth=0.5,
                    linestyle="dotted",
            ...
            row_guides = [
                np.column_stack([np.arange(-1, nbells + 3), row * np.ones(nbells + 4)])
                for row in range(
                    plot * rows_per_plot + min_plot_change,
                    (plot + 1) * rows_per_plot + min_plot_change + 1,
                )
            ]
            # for row in range(min_plot_change, max_plot_change):
            #    ax.plot(np.arange(-1,nbells+3), row*np.ones(nbells+4), c = 'black',
            #            linewidth = 0.5, linestyle = 'dotted', zorder = 0)
            line_collection = LineCollection(
                row_guides, color="black", linewidth=0.5, linestyle="dotted", zorder=0
            )
            ax.add_collection(line_collection)

            plt.gca().invert_yaxis()
            ax.set_ylim(
                (plot + 1) * rows_per_plot + min_plot_change,
                plot * rows_per_plot + min_plot_change,
            )
